...
ui <- page_fluid(
  ...
  titlePanel("Consortium Innovation Dashboard (EIS 2024)"),
  ...
  layout_columns(
    ...
    card(
      ...
      card_header("Consortium table"),
      DTOutput("partners_table")
    ),
    
    # ---------------- Right column ----------------
    card(
      card_header("Map + summaries"),
      plotlyOutput("map", height = 520),
      
      hr(),
      
      layout_columns(
        card(
          card_header("By innovation level"),
          DTOutput("level_summary")
        ),
        card(
          card_header("By country"),
          DTOutput("country_summary")
        )
      )
    )
  )
...
server <- function(input, output, session) {
  ...
  output$kpis <- renderUI({
    ...
    layout_columns(
      value_box(title = "Partners", value = total_partners),
      value_box(title = "Countries", value = countries_n),
      value_box(title = "Leaders", value = leaders),
      value_box(title = "Strong", value = strong),
      value_box(title = "Moderate", value = moderate),
      value_box(title = "Emerging", value = emerging)
    )
  })
  ...
  output$partners_table <- renderDT({
    df <- partners_enriched() |>
      select(partner, country, iso3, innovation_level)
